...
Initially when Postmark account is created in "test mode" you can only send emails to the same domain as your Member Matters domain. This domain limitation may create problems if your test account emails use a different domain.  You can add the ability to send from a specific domain to an arbitrary domains by adding a domain "sender signature" in the Postmark account.  Postmark errors are not shown to members: a failed send is logged (see [Logs](#logs), and the Celery worker's output) and recorded in the member's event log as "Email NOT sent ...".
